...
(function () {
    ...
    freeboard.loadWidgetPlugin({
		...
		var toggleWidget = function (settings) {
				id = '_' + Math.random().toString(36).substr(2, 9)
				var self = this;
				var titleElement = $('<h2 class="section-title"></h2>');
				var stateElement = $('<div class="indicator-text"></div>');
				//var indicatorElement = $('<button onclick="myFunction()">Click me</button>');
				var indicatorElement = $('<p class="indicator-text" align="center" id="'+ settings.uuid + '"><img src="on.png" width="90" height="90"></p>');
				var currentSettings = settings;
				var isOn = false;
				var onText;
				var offText;
				...
				client = new Paho.MQTT.Client(currentSettings.host, Number(currentSettings.port), currentSettings.uuid);
				...
				this.render = function (element) {
						$(element).append(titleElement).append(indicatorElement).append(stateElement);
						document.getElementById(currentSettings.uuid).onclick = function() {
							message = new Paho.MQTT.Message(currentSettings.message);
							message.destinationName = currentSettings.topic;
							client.send(message);
						};
				}
		...
		freeboard.loadWidgetPlugin({
				...
				settings: [
					...
					},
					{
							...
							type: "text",
							default_value: "broker.mqttdashboard.com"
					},
					{
							name: "port",
							display_name: "Port",
					...
					{
							name: "message",
							display_name: " Publishing Message",
							type: "text"
					}

				],
				newInstance: function (settings, newInstanceCallback) {
						newInstanceCallback(new toggleWidget(settings));
				}
		});
